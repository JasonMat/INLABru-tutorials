...
author: "David Borchers and Finn Lindgren (Comments and edits from JM)"
...
the `R` package `spatstat`. We will assume that all objects have been pre-formatted in the necessary spatial data frame format, and we will also assume that the 2D mesh has been pre-defined. We will set up a two-dimensional LGCP to estimate Gorilla abundance.
...
an SPDE `+ Intercept(1)` on the right. Please use the SPDE prior specification stated below (*This is probably driven by looking at the scales involved in the above map of the mesh, but why is the quantile so small, at 0.01?*). The 'coordinates' component of the model will accept the information contained in the 'mesh' object. 

```{r eval=FALSE}
matern<-inla.spde2.pcmatern(mesh, prior.range = c(5, 0.01), prior.sigma = c(0.1, 0.01))
mdl<-coordinates~anyName(coordinates, model=matern) + Intercept(1)
  
```
*Note that, in the definition of the model, the name provided to the smooth can be arbitrary (here, 'anyName'). I suppose this defines a new function, on the fly, providing the argument 'coordinates' and the body 'model'? In contrast, the component 'coordinates' is hard-wired and used as a named argument in the domain definition below, when the 'lgcp' model is called.*
...
using the `samplers` argument. Note that the necessary point and polygon data frames are already formatted as spatial data frames in the loaded data set. 

```{r eval=FALSE, warning=FALSE, message=FALSE}

fit.spde<-lgcp(mdl, data=nests, samplers=boundary, domain=list(coordinates=mesh))
  
```

3. Call `predict( )`, passing it the fitted model from 2., locations at which to predict and
an appropriate predictor specification. The locations at which to predict should be a 
`SpatialPixelsDataFrame` covering the mesh obtained by calling  `pixels(mesh)`.


```{r eval=FALSE, warning=FALSE, message=FALSE}

pred_Frame<-pixels(mesh, nx = 50, ny = 50, mask = FALSE)
pred_spde<-predict(fit.spde, pred_Frame,~ exp(anyName + Intercept))
...
Given some generous interval boundaries (500, 800) for lambda we can estimate the posterior abundance distribution via (Question: Does this version of the posterior contain parameter uncertainty? Or is lambda calculated on the basis of some median from the preceding summary step, above? Look below, in the final output of the tutorial, at the comparison with the plugin posterior. The posterior produced here is wider than the plugin. This indicates that the `lambda=sum()` operation below is doing something more clever than aggregating unit rates from a single surface).
