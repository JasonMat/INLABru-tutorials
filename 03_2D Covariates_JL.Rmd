...
author: "David Borchers and Finn Lindgren (scrawled over by Jonathan Lee)"
...
!JL!
gcov holds a bunch of attributes like aspect as well as the elevation and vegetation data. They're all SpatialPixelsDataFrames, which i'm not familiar with, but seems to let you have a 2d grid, which can automatically yield the coordinates (as per its CRS) of each cell via the "coordinates" function.
!JL!
...
Maybe this is obvious, but 'coordinates' can be another word, as long as its consistent.
...
The `predict` function of `inlabru` takes into its `data` argument a `SpatialPointsDataFrame`, 
...
I find the section below confusing, since it seems to get the vegetation data from the mesh. I can see how this could be used to create the SpatialPixelsDataFrame, but the mesh doesn't seem to have the data in it.
...
```
NOTE: the behaviour of DIC and WAIC is currently a bit unclear, in particular for experimental
mode, and is being investigated.

Classic mode:
```{r echo=FALSE}
knitr::kable(cbind(dplyr::tribble(
~Model, ~DIC, ~Delta.DIC,
...
"fit1",	2672.234,	0.0000,
"fit3",	2949.498,	277.2636,
"fit2",	3040.140,	367.9051)))
```
...
knitr::kable(cbind(dplyr::tribble(
  ...
  dplyr::tribble(
    ~Model,	~WAIC,	~Delta.WAIC,
    "fit1",	1373.241,	0.0000,
    "fit3",	1584.543,	211.3021,
    "fit2",	1636.864,	263.6225)))
```
...
So, my understanding is that the fitting always needs a function, but INLA[bru] has a whitelist of object types that it can automatically grab the function out of. I assume this means the two cases are identical in terms of speed (assuming the functions take about the same time).
